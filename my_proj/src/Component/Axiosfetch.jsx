...
const Postdata = () => {
  ...
  const delData = async (userDetails) => {
    console.log('data is delete')
    console.log("User id h:", userDetails.id)
    await axios({
      url: `https://6a489aaca033dcb98d64b0ff.mockapi.io/users/${userDetails.id}`,
      method: 'delete',
      data: null
  ...
  return (
    <>
      ...
      {users.map((userDetails) => {
        return (
          <li key={userDetails.id} className='bg-gray-700 p-6 rounded-lg my-2 text-center list-none '>
            <p>Name:{userDetails.name}</p>
            <p>Age:{userDetails.age}</p>
            <button className='btn m-2' onClick={() => setUserdetails({ ...userDetails, editMode: true })}>Edit</button>
            <button className='btn m-2' onClick={() => delData(userDetails)}>Delete</button>
          </li>
